...
var TaroEntity = TaroObject.extend({
	...
	init: function (defaultData = {}) {
		...
		this._keyFrames = [];
		this.prevKeyFrame = [taro.now, [this._translate.x, this._translate.y, this._rotate.z]];
		...
		if (this._parent && this._parent._tileWidth !== undefined && this._parent._tileHeight !== undefined) {
			...
			if (lockAspect) {
				if (this._texture) {
					...
					TaroEntity.prototype.log('Cannot set height based on texture aspect ratio and new width because no texture is currently assigned to the entity!', 'error');
				}
			}
		} else {
			TaroEntity.prototype.log('Cannot set width by tile because the entity is not currently mounted to a tile map or the tile map has no tileWidth or tileHeight values.', 'warning');
		}
		...
		if (this._parent && this._parent._tileWidth !== undefined && this._parent._tileHeight !== undefined) {
			...
			if (lockAspect) {
				if (this._texture) {
					...
					TaroEntity.prototype.log('Cannot set width based on texture aspect ratio and new height because no texture is currently assigned to the entity!', 'error');
				}
			}
		} else {
			TaroEntity.prototype.log('Cannot set height by tile because the entity is not currently mounted to a tile map or the tile map has no tileWidth or tileHeight values.', 'warning');
		}
	...
	occupyTile: function (x, y, width, height) {
		...
		if (this._parent && this._parent.TaroTileMap2d) {
			if (x !== undefined && y !== undefined) {
				...
				var trPoint = new TaroPoint3d(this._translate.x - (this._tileWidth / 2 - 0.5) * this._parent._tileWidth, this._translate.y - (this._tileHeight / 2 - 0.5) * this._parent._tileHeight, 0);
	...
	size3d: function (x, y, z) {
		TaroEntity.prototype.log('size3d has been renamed to bounds3d but is exactly the same so please search/replace your code to update calls.', 'warning');
	},
	...
	cellById: function (val) {
		if (val !== undefined) {
			if (this._texture) {
				...
				TaroEntity.prototype.log(`Could not find the cell id "${val}" in the assigned entity texture ${tex.id()}, please check your sprite sheet (texture) cell definition to ensure the cell id "${val}" has been assigned to a cell!`, 'error');
			} else {
				TaroEntity.prototype.log('Cannot assign cell index from cell ID until an TaroSpriteSheet has been set as the texture for this entity. Please set the texture before calling cellById().', 'error');
			}
		}
	...
	screenPosition: function () {
		return new TaroPoint3d(Math.floor((this._worldMatrix.matrix[2] - taro._currentCamera._translate.x) * taro._currentCamera._scale.x + taro._bounds2d.x2), Math.floor((this._worldMatrix.matrix[5] - taro._currentCamera._translate.y) * taro._currentCamera._scale.y + taro._bounds2d.y2), 0);
	},
	...
	localAabb: function (recalculate) {
		if (!this._localAabb || recalculate) {
			var aabb = this.aabb();
			this._localAabb = new TaroRect(-Math.floor(aabb.width / 2), -Math.floor(aabb.height / 2), Math.floor(aabb.width), Math.floor(aabb.height));
		}
	...
	isBehind: function (otherObject) {
		...
		var otherMax = new TaroPoint3d(otherX + otherG3d.x / 2, otherY + otherG3d.y / 2, otherObject._translate.z + otherG3d.z);
	...
	update: function (ctx, tickDelta, isForOrphans) {
		...
		// if (taro.updateCount[category] == undefined)
		// 	taro.updateCount[category] = 0;
		// taro.updateCount[category]++;
		...
		if (this._deathTime !== undefined && this._deathTime <= taro._tickStart) {
			...
			if (this._bornTime === undefined || taro._currentTime >= this._bornTime) {
				...
				if (!isForOrphans && taro.gameLoopTickHasExecuted) {
	...
	playEffect: function (type) {
		if (this._stats && this._stats.effects && this._stats.effects[type]) {
			...
			if (taro.isServer) {
				...
				if (effect.runScript) {
					taro.script.runScript(effect.runScript, {});
				}
			}
		}
	},
	...
	destroy: function (destroyOrphan) {
		 ...
		 * @event TaroEntity#destroyed
		 * @param {TaroEntity} The entity that has been destroyed.
		 */
		for (var region in taro.regionManager.entitiesInRegion) {
			delete taro.regionManager.entitiesInRegion[region][this.id()];
		}

		TaroEntity.prototype.log(`entity destroyed ${this.id()} category: ${this._category} ${this._stats ? this._stats.name : ''}`);
	...
	clearAllPointers: function () {
		var keysToDelete = ['ability', 'animation', 'attribute', 'inventory', 'minimapUnit', 'unitUi', '_aabb', '_bounds2d', '_bounds3d', '_rotate', '_velocity'];
	...
	teleportTo: function (x, y, rotate) {
		...
		if (taro.isServer) {
			taro.network.send('teleport', { entityId: this.id(), position: [x, y] });
	...
	translateToTile: function (x, y, z) {
		if (this._parent && this._parent._tileWidth !== undefined && this._parent._tileHeight !== undefined) {
			...
			TaroEntity.prototype.log('Cannot translate to tile because the entity is not currently mounted to a tile map or the tile map has no tileWidth or tileHeight values.', 'warning');
		}
	...
	translate: function () {
		if (arguments.length) {
			TaroEntity.prototype.log('You called translate with arguments, did you mean translateTo or translateBy instead of translate?', 'warning');
		}
	...
	rotate: function () {
		if (arguments.length) {
			TaroEntity.prototype.log('You called rotate with arguments, did you mean rotateTo or rotateBy instead of rotate?', 'warning');
		}
	...
	scale: function () {
		if (arguments.length) {
			TaroEntity.prototype.log('You called scale with arguments, did you mean scaleTo or scaleBy instead of scale?', 'warning');
		}
	...
	updateTransform: function () {
		...
		if (this._mode === 1) {
			if (!isNaN(this._translate.x) && !isNaN(this._translate.y)) {
				// iso translation
				var isoPoint = (this._translateIso = new TaroPoint3d(this._translate.x, this._translate.y, this._translate.z + this._bounds3d.z / 2).toIso());
				...
				if (taro.chat) taro.chat.postMessage({ text: `localMatrix translate error: ${this._category}${this._stats}` ? this._stats.name : '' });
			}
		}
			...
			for (var i = 0; i < queuedData.length; i++) {
				...
				for (attrName in data) {
					...
					switch (attrName) {
						...
						case 'isBeingUsed':
							// this case is in taroEntity.js instead of item.js, because if it's in item.js,
						...
						case 'currentItemIndex':
							...
							if (taro.isClient) {
								...
								if (selectedItemId && taro.$(selectedItemId)) {

									// tell client this item is selected
									taro.$(selectedItemId).setState('selected');
								}
							}
							break;
					...
					if (taro.isServer) {
						...
						switch (attrName) {
							...
							case 'effect':
								...
								if (this == taro.client.selectedUnit || (this._category == 'item' && this.getOwnerUnit() == taro.client.selectedUnit))
									return;
	...
	streamCreateData: function (clientId) {
		if (taro.isServer) {
			...
			var keys = taro.server.keysToRemoveBeforeSend.slice();
	...
	streamCreate: function (clientId) {
		...
		if (this._parent && taro.network.stream._streamClientCreated !== undefined) {
			...
			taro.network.send('_taroStreamCreate', [this.classId(), thisId, this._parent.id(), this._streamSectionData, streamCreateData], clientId);
	...
	_processTransform: function () {
		if (
			...
			(this._stats.currentBody && this._stats.currentBody.type == 'spriteOnly' && (this.getOwnerUnit && this.getOwnerUnit()) == taro.client.selectedUnit)
		) {
			return;
		}
		...
		if (
			...
			// 1. we're using cspMovement (experimental) for my own unit OR
				(taro.game.cspEnabled && taro.client.selectedUnit == this) ||
			...
			// 		x = this.interpolateValue(this.prevPhysicsFrame[1][0], this.nextPhysicsFrame[1][0], this.prevPhysicsFrame[0], taro._currentTime, this.nextPhysicsFrame[0]);
			// 		y = this.interpolateValue(this.prevPhysicsFrame[1][1], this.nextPhysicsFrame[1][1], this.prevPhysicsFrame[0], taro._currentTime, this.nextPhysicsFrame[0]);

			// 		if (this == taro.client.selectedUnit) {
			// 			rotate = this.interpolateValue(this.prevPhysicsFrame[1][2], this.nextPhysicsFrame[1][2], this.prevPhysicsFrame[0], taro._currentTime, this.nextPhysicsFrame[0]);
			// 		}
		...
		if (prevTransform != undefined && nextTransform != undefined &&
			...
			targetX = this.interpolateValue(prevTransform[0], nextTransform[0], prevKeyFrame[0], taro._currentTime, nextKeyFrame[0]);
			targetY = this.interpolateValue(prevTransform[1], nextTransform[1], prevKeyFrame[0], taro._currentTime, nextKeyFrame[0]);
			...
			if (this == taro.client.selectedUnit && this.angleToTarget != undefined && !isNaN(this.angleToTarget) && this._stats.controls && this._stats.controls.mouseBehaviour.rotateToFaceMouseCursor && this._stats.currentBody && !this._stats.currentBody.fixedRotation) {
				targetRotate = this.angleToTarget;
			} else {
				targetRotate = this.interpolateValue(startValue, endValue, prevKeyFrame[0], taro._currentTime, nextKeyFrame[0]);
			}
			...
			if (this == taro.client.selectedUnit && this.lastReconciledAt != prevKeyFrame[0]) {
			...
			// if (taro.physics && taro.game.cspEnabled && this != taro.client.selectedUnit) {
			// 	xDiff = targetX - x;
			// 	yDiff = targetY - y;
			// 	x += xDiff / 2;
			// 	y += yDiff / 2;
			// } else if (!taro.physics || !taro.game.cspEnabled || this != taro.client.selectedUnit) {
			...
			// 	let timeElapsed = (taro._currentTime-this.previous_currentTime).toFixed(0)
			// 	console.log(
			// 		taro.snapshots.length, 
			// 		'x', prevTransform[0], x.toFixed(0), '(' + distanceTraveled.toFixed(1) + ')', nextTransform[0],
			// 		'time', prevKeyFrame[0], taro._currentTime.toFixed(0), '(' + timeElapsed + 'ms '+ ((taro._currentTime - prevKeyFrame[0]) / (nextKeyFrame[0] - prevKeyFrame[0]) * 100).toFixed(0) +'%)', nextKeyFrame[0], 
			// 		"speed", (distanceTraveled/timeElapsed).toFixed(2)
			// 	);
			// 	this.previousX = x;
			// 	this.previous_currentTime = taro._currentTime;
			// }
		}
