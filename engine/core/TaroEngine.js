...
var TaroEngine = TaroEntity.extend({
	...
	init: function (options) {
		...
		TaroEngine.prototype.log('initializing IGE engine');
		if (taroConfig.debug) {
	...
	engineStep: function (timeStamp, ctx) {
		...
		if (self._state) {
			...
			var timeElapsed = taro.now - taro._lastPhysicsTickAt;			
			if (// physics update should execute as soon as gameloop has executed in order to stream the accurate, latest translation data computed from physics update
				taro.physics && (
					taro.gameLoopTickHasExecuted || // don't ask. - Jaeyun
				...
				taro.physicsTickHasExecuted = true;
			}

			if (taro.isServer) {
				if (taro.gameLoopTickHasExecuted)
					taro.trigger.fire('frameTick');
			...
			taro.action.actionProfiler = {}
			taro.trigger.triggerProfiler = {}
			...
			if (taro.isServer) {
				if (taro.now - self.lastCheckedAt > 1000) {
					...
					if (playerCount <= 0) {
						...
						const serverTier = taro.server.tier;
						...
						if ((serverTier === '1' || serverTier === '2') && gameTier !== '4' && taro.now - self.serverEmptySince > self.emptyTimeLimit) {
		...
		taro.physicsTickHasExecuted = false;
