 ...
 */
var TaroViewport = TaroEntity.extend([
	{ extension: TaroUiStyleExtension, overwrite: true },
	{ extension: TaroUiPositionExtension, overwrite: true }
], {
	classId: 'TaroViewport',
	TaroViewport: true,
	...
	init: function (options) {
		...
		if (options) {
			...
			if (options && options.scaleToWidth && options.scaleToHeight) {
				...
				this._lockDimension = new TaroPoint3d(options.scaleToWidth, options.scaleToHeight, 0);
			}
		}

		// Setup default objects
		this._bounds2d = new TaroPoint3d(width || taro._bounds2d.x, height || taro._bounds2d.y, 0);
		this.camera = new TaroCamera(this);
	...
	minimumVisibleArea: function (width, height) {
		// Store the w/h we want to lock to
		this._lockDimension = new TaroPoint3d(width, height, 0);
		if (taro.isClient) {
			this._resizeEvent({});
		}
	...
	viewArea: function () {
		var aabb = this.aabb();
		var camTrans = this.camera._translate;
		var camScale = this.camera._scale;
		var width = aabb.width * (1 / camScale.x);
		var height = aabb.height * (1 / camScale.y);
	...
	update: function (ctx, tickDelta) {
		...
		if (this._scene) {
			// Store the viewport camera in the main taro so that
			// down the scenegraph we can choose to negate the camera
			// transform effects
			taro._currentCamera = this.camera;
			taro._currentViewport = this;

			this._scene._parent = this;

			this.camera.update();
			TaroEntity.prototype.update.call(this, ctx, tickDelta);
	...
	tick: function (ctx, scene) {
		// don't render minimap viewport if minimap turned off
		if (this._id == 'vpMiniMap' && taro.isClient && !taro.client.miniMapEnabled) return;
		// Check if we have a scene attached to this viewport
		if (this._scene) {
			// Store the viewport camera in the main taro so that
			// down the scenegraph we can choose to negate the camera
			...
			if (taro.isClient && taro.client.clearEveryFrame) {
				ctx.clearRect(0, 0, this._bounds2d.x, this._bounds2d.y);
			}

			// Clip the context so we only draw "inside" the viewport area
			if (taro.isClient && taro.client.viewportClippingEnabled) {
				if (this._clipping || this._borderColor) {
					ctx.beginPath();
					ctx.rect(0, 0, this._bounds2d.x / taro._scale.x, this._bounds2d.y / taro._scale.x);
			...
			ctx.translate(((this._bounds2d.x / 2) | 0) + taro._translate.x, ((this._bounds2d.y / 2) | 0) + taro._translate.y);
			/* ctx.translate(taro._translate.x, taro._translate.y); */
			if (taro._scale.x !== 1 || taro._scale.y !== 1) {
				ctx.scale(taro._scale.x, taro._scale.y);
			}
			...
			if (this._drawGuides && ctx === taro._ctx) {
				ctx.save();
			...
			if (this._drawBounds && ctx === taro._ctx) {
			...
			if (this._drawMouse && ctx === taro._ctx) {
				ctx.save();
	...
	/**
	 * Returns the screen position of the viewport as an TaroPoint3d where x is the
	...
	screenPosition: function () {
		return new TaroPoint3d(
			Math.floor(this._worldMatrix.matrix[2] + taro._bounds2d.x2),
			Math.floor(this._worldMatrix.matrix[5] + taro._bounds2d.y2),
			0
		);
	},
	...
	paintAabbs: function (ctx, rootObject, index) {
		...
		var ga;
		var r3d;
		var xl1; var xl2; var xl3; var xl4; var xl5; var xl6;
		var bf1; var bf2; var bf3; var bf4;
		var tf1; var tf2; var tf3; var tf4;
		...
		if (arr) {
			...
			while (arrCount--) {
				...
				if (obj._shouldRender !== false) {
					if (obj._classId !== 'TaroScene2d' && (!this._drawBoundsLimitId && !this._drawBoundsLimitCategory) || ((this._drawBoundsLimitId && (this._drawBoundsLimitId instanceof Array ? this._drawBoundsLimitId.indexOf(obj.id()) > -1 : this._drawBoundsLimitId === obj.id())) || (this._drawBoundsLimitCategory && this._drawBoundsLimitCategory === obj.category()))) {
							...
							if (aabb) {
								if (obj._drawBounds || obj._drawBounds === undefined) {
									...
									if (obj._parent && obj._parent._mountMode === 1) {
										...
										r3d = obj._bounds3d;
										xl1 = new TaroPoint3d(-(r3d.x / 2), 0, 0).toIso();
										xl2 = new TaroPoint3d(+(r3d.x / 2), 0, 0).toIso();
										xl3 = new TaroPoint3d(0, -(r3d.y / 2), 0).toIso();
										xl4 = new TaroPoint3d(0, +(r3d.y / 2), 0).toIso();
										xl5 = new TaroPoint3d(0, 0, -(r3d.z / 2)).toIso();
										xl6 = new TaroPoint3d(0, 0, +(r3d.z / 2)).toIso();
										// Bottom face
										bf1 = new TaroPoint3d(-(r3d.x / 2), -(r3d.y / 2), -(r3d.z / 2)).toIso();
										bf2 = new TaroPoint3d(+(r3d.x / 2), -(r3d.y / 2), -(r3d.z / 2)).toIso();
										bf3 = new TaroPoint3d(+(r3d.x / 2), +(r3d.y / 2), -(r3d.z / 2)).toIso();
										bf4 = new TaroPoint3d(-(r3d.x / 2), +(r3d.y / 2), -(r3d.z / 2)).toIso();
										// Top face
										tf1 = new TaroPoint3d(-(r3d.x / 2), -(r3d.y / 2), (r3d.z / 2)).toIso();
										tf2 = new TaroPoint3d(+(r3d.x / 2), -(r3d.y / 2), (r3d.z / 2)).toIso();
										tf3 = new TaroPoint3d(+(r3d.x / 2), +(r3d.y / 2), (r3d.z / 2)).toIso();
										tf4 = new TaroPoint3d(-(r3d.x / 2), +(r3d.y / 2), (r3d.z / 2)).toIso();
