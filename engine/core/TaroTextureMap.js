 ...
 */
var TaroTextureMap = TaroTileMap2d.extend({
	classId: 'TaroTextureMap',

	init: function (tileWidth, tileHeight) {
		TaroTileMap2d.prototype.init.call(this, tileWidth, tileHeight);
		this.map = new TaroMap2d();
		this._textureList = [];
		this._renderCenter = new TaroPoint3d(0, 0, 0);
	 ...
	 * exists in the other.
	 * @param {TaroTileMap2d} entity The other map to read map data from.
	 * @return {*}
	 */
	 ...
	 */
	tick: function (ctx) {
		if (taro.isClient && !taro.client.mapRenderEnabled) return;

		// TODO: This is being called at the wrong time, drawing children before this parent! FIX THIS
		// Run the TaroTileMap2d tick method
		TaroTileMap2d.prototype.tick.call(this, ctx);
	...
	_drawSectionsToCtx: function (ctx) {
		...
		var viewArea = taro._currentViewport.viewArea();
			...
			if (this._sections.hasOwnProperty(x)) {
				for (y in this._sections[x]) {
					if (this._sections[x].hasOwnProperty(y)) {
						...
						sectionAbsX = this._translate.x + sectionRenderX - taro._currentCamera._translate.x;
						sectionAbsY = this._translate.y + sectionRenderY - taro._currentCamera._translate.y;
	 ...
	 * @param {TaroRect=} rect The rectangular area to limit drawing to.
	...
	_renderTile: function (ctx, x, y, tileData, tileEntity, rect, sectionX, sectionY) {
		...
		// Paint the texture
		if (texture) {
			texture.render(ctx, tileEntity, taro._tickDelta);
		}
		ctx.restore();
...
if (typeof (module) !== 'undefined' && typeof (module.exports) !== 'undefined') { module.exports = TaroTextureMap; }
