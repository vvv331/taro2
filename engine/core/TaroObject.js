 ...
 */
var TaroObject = TaroEventingClass.extend({
	classId: 'TaroObject',
		...
		if (id !== undefined) {
			...
			if (id !== this._id) {
				...
				if (taro._register[id]) {
					...
					if (this._id && taro._register[this._id]) {
						// Unregister the old ID before setting this new one
						taro.unRegister(this);
					}
	 ...
	 *     var entityArray = taro.$$('categoryName');
			...
			if (groupName instanceof Array) {
				...
				while (groupItemCount--) {
					if (!this._groups || this._groups.indexOf(groupName[groupItemCount]) === -1) {
						...
						taro.groupRegister(this, groupName[groupItemCount]);
					}
				}
			} else {
				if (!this._groups || this._groups.indexOf(groupName) === -1) {
					...
					// Now register this object with the group it has been assigned
					taro.groupRegister(this, groupName);
				}
			}
		}
			...
			while (arrCount--) {
				...
				if (groupName instanceof Array) {
					...
					while (groupNameCount--) {
						...
						taro.groupUnRegister(this, groupName[groupNameCount]);
					}
				} else {
					this._groups.pull(groupName);

					// Now un-register this object with the group it has been assigned
					taro.groupUnRegister(this, groupName);
				}
			}
		}
	...
	removeAllGroups: function () {
		if (this._groups) {
			...
			while (arrCount--) {
				taro.groupUnRegister(this, arr[arrCount]);
			}
	 ...
	 * (TaroTileMap2d) instances, when enabled you will see the tile x and y
	 ...
	 *     var entity1 = new TaroEntity(),
	 *         entity2 = new TaroEntity(),
	 *         entity3 = new TaroEntity();
	 *
	...
	viewCheckChildren: function () {
		if (taro._currentViewport) {
			var arr = this._children;
			var arrCount = arr.length;
			var vpViewArea = taro._currentViewport.viewArea();
			var item;

			while (arrCount--) {
				item = arr[arrCount];
				if (item._category === 'minimapUnit') {
					item._inView = true;
				} else {
	...
	update: function (ctx, tickDelta) {
		...
		if (this._alive) {
			if (taro.isClient && !this._inView) return;
					...
					while (arrCount--) {
						...
						if (!arr[arrCount]._newBorn) {
							...
							if (renderChildBelowUnit) {
								...
								// dont render item if it is already rendered below unit
								renderChild = true && !arr[arrCount]._renderBelowUnit;
							}
							if (renderChild) {
								// ctx.save();
								ts = new Date().getTime();
	...
	objSave: function () {
		return { taroClass: this.classId(), data: this._objSaveReassign(this, []) };
	},
	...
	loadGraph: function (obj) {
		if (obj.taroClass && obj.data) {
			// Create a new class instance
			var classInstance = taro.newClassInstance(obj.taroClass);
			var newId;
			var childArr;
			var childIndex;
			var parentId;
