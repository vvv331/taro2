		...
		if (commandName === '_snapshot') {
			...
			if (snapshot.length) {
				...
				if (Object.keys(obj).length) {
					...
					if (taro._currentTime > newSnapshotTimestamp || taro._currentTime < newSnapshotTimestamp - 100) {
						...
						taro.timeDiscrepancy = newSnapshotTimestamp - Date.now() - 50;						
					} else {
						// rubberband currentTime to be nextSnapshot's timestamp - 50ms
						taro.timeDiscrepancy += ((newSnapshotTimestamp - Date.now() - 50) - taro.timeDiscrepancy) / 5; 
					}
				}
			}
		} else {
