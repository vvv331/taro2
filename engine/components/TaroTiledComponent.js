		...
		if (taro.isClient) {
			onLoadFunc = function (textures, tileSetCount, tileSetItem) {
				return function () {
					...
					var cs = new TaroCellSheet(imageUrl, this.width * scaleFactor.x / tileSetItem.tilewidth, this.height * scaleFactor.y / tileSetItem.tileheight, taro.scaleMapDetails.shouldScaleTilesheet)
						.id(tileSetItem.name)
						.on('loaded', function () {
