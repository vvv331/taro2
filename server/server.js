...
var Server = TaroClass.extend({
		...
		taro.network.start(self.port, function (data) {
			...
			promise.then((game) => {
				...
				if (game.data.defaultData && !isNaN(game.data.defaultData.frameRate)) {
					taro._physicsTickRate = Math.max(15, Math.min(parseInt(game.data.defaultData.frameRate), 60)); // keep fps range between 15 and 60
				}
					...
					if (success) {
						...
						var logInterval = setInterval(function () {
							...
							if (taro.isServer && self.developerClientIds.length) {
								...
								taro.variable.devLogs.status = taro.server.getStatus();
								const sendErrors = Object.keys(taro.script.errorLogs).length;

								self.developerClientIds.forEach(
									id => {
										taro.network.send('devLogs', taro.variable.devLogs, id);
	...
	defineNetworkEvents: function () {
		...
		taro.network.define('teleport', self._onSomeBullshit);
		...
		taro.network.define('setOwner', self._setOwner);
